...
プロジェクトの開発の流れを検討する際の参考にしてください（最初は図の要素一つ一つを理解する必要はありません。工程毎に成果物やタスクがあることが掴めれば十分です）。

- [Nablarchを使用した開発の流れ](../サンプルアプリケーション/設計書/Nablarchを使用した開発の流れ.xlsx)
...
セキュリティ設計については、IPAの「安全なウェブサイトの作り方」を参照してください。特に「セキュリティ実装 チェックリスト」について全ての項目が「根本的解決」となるように設計を行います。重複はありますが、「OWASP Top10」もチェックしておきます。
...
Nablarchの使用経験がない場合は、ここでサンプルアプリケーションを参照し、実際のプロジェクト成果物を確認しておきましょう。
...
サンプルアプリケーションの成果物を、何の検討もせずにプロジェクトにコピーして流用するのはやめましょう。
...
必ず、自身のプロジェクトの要件と特性を踏まえた上で、標準化やガイドを新規に作成するようにしてください。
...
特にアプリケーション開発標準(*1)を参照してください。ここにはDB設計標準やUI標準等、重要な設計標準ドキュメントが含まれています。
...
上記ドキュメントを参照しながら、プロジェクト用にテスト項目を整備します。
